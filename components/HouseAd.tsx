...
export default function HouseAd({ orientation }: { orientation: Orientation }) {
  if (orientation === 'horizontal') {
    return (
      <div className="flex h-full w-full items-center justify-between rounded-2xl border border-white/10 bg-gradient-to-r from-[#1f1c2c] via-[#2b5876] to-[#4e4376] px-3 text-[11px] text-white">
        <span className="font-semibold tracking-[0.25em] uppercase">Sponsored</span>
        <span className="mx-2 text-xs font-medium opacity-90">Inventory not filled</span>
  ...
  return (
    <div className="group flex h-full w-full flex-col items-center justify-center gap-4 rounded-2xl border border-white/15 bg-gradient-to-r from-[#1f1c2c] via-[#2b5876] to-[#4e4376] px-4 py-4 text-white">
