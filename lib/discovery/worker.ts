...
import { dailymotionPageLoader } from './dailymotion'
...
export async function runDiscoveryBatch(db: Db) {
  ...
  const { finalizeVideoIngest } = await import('../ingest/videos')
    ...
    for (const spec of seeds) await enqueue(db, spec)
    if (process.env.RANDOM_DM_DISCOVERY_ENABLED === '1') {
      for (const spec of seeds.slice(0, 6)) if (spec.kind === 'search') await enqueue(db, {
      ...
      for (let i = 0; i < 4; i++) {
        ...
        await enqueue(db, { kind: 'dailymotion', category: categories[Math.floor(random() * categories.length)],
    ...
    for (const channelId of [...new Set(trends.map(x => String(x.channelId)))].slice(0, 4)) await enqueue(db, { kind: 'channel', channelId })
    ...
    const report = await runExploration({ db, quota, random, loadPage: (task, signal, permit) => task.spec.kind === 'dailymotion' ? dailymotion(task, signal, permit) : youtube ? youtube(task, signal, permit) : Promise.reject(new Error('youtube-unconfigured')),
