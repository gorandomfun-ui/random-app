...
const local_ingest_1 = require("./local-ingest");
...
<body>
  ...
  <fieldset>
    ...
    <div>
      ${local_ingest_1.ALL_CATEGORIES.map((cat) => `<label><input type="checkbox" class="cat-checkbox" value="${cat}" checked /> ${cat}</label>`).join('')}
    </div>
  </fieldset>
  ...
  <div class="quick-grid">
    ...
    ${local_ingest_1.ALL_CATEGORIES.map((cat) => `<button type="button" class="quick" data-cat="${cat}">Run ${cat}</button>`).join('')}
  </div>
...
<script>
const categories = ${JSON.stringify(local_ingest_1.ALL_CATEGORIES)}
        ...
        req.on('end', async () => {
            try {
                ...
                const categories = Array.isArray(body.categories) && body.categories.length
                    ? body.categories.filter((value) => local_ingest_1.ALL_CATEGORIES.includes(value))
                    : local_ingest_1.ALL_CATEGORIES.slice();
                const logs = [];
                running = true;
                const summary = await (0, local_ingest_1.runIngest)({
                    categories,
                    dryRun: body.dryRun,
